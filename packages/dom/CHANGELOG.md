...
- 88151de: ## Breaking Changes

  ### Complete v2 rewrite with `# @logosdx/dom selector API

  The DOM package has been completely rewritten with a new chainable API built around the `# @logosdx/dom selector function and `DomCollection` class. The previous API surface has been replaced entirely.
  ...
  **After:**
  ...
  - `feat(dom):` Add `# @logosdx/dom selector with `DomCollection` chainable callable namespaces
