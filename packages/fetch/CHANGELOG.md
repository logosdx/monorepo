...
- 2f9c85c: ## Breaking Changes
  ...
  const api = new FetchEngine({
    modifyConfig: (config, state) => ({
      ...config,
      headers: { ...config.headers, Authorization: state.token },
    }),
  });
  ...
  engine.hooks.add("beforeRequest", (url, opts, ctx) => {
    ctx.args(url, {
      ...opts,
      headers: { ...opts.headers, Authorization: engine.state.get().token },
    });
  });
  ```
