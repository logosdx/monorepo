...
describe('@logosdx/fetch: caching', async () => {
    ...
    const { testUrl } = await makeTestStubs(4121);
     ...
     it('should enable caching for GET requests by default', async () => {
        ...
        api.on('cache-hit', (data) => hitEvents.push(data.path!));
        api.on('cache-miss', (data) => missEvents.push(data.path!));
        api.on('cache-set', (data) => setEvents.push(data.path!));
     ...
     it('should disable caching when cachePolicy is false', async () => {
        ...
        api.on('cache-hit', mock);
        api.on('cache-miss', mock);
     ...
     it('should configure methods for caching', async () => {
        ...
        api.on('cache-hit', () => mock('hit'));
        api.on('cache-miss', () => mock('miss'));
     ...
     it('should emit cache-set when storing value', async () => {
     ...
     it('should disable caching for routes matching a rule with enabled: false', async () => {
        ...
        api.on('cache-miss', () => events.push('miss'));
        api.on('cache-hit', () => events.push('hit'));
     ...
     it('cache check happens before deduplication', async () => {
        ...
        api.on('dedupe-start', () => dedupeStarts.push('start'));
     ...
     it('should return stale value immediately and trigger background revalidation', async () => {
        ...
        api.on('cache-stale', () => staleEvents.push('stale'));
        api.on('cache-revalidate', () => revalidateEvents.push('revalidate'));
     ...
     it('should emit cache-stale with isStale: true', async () => {
     ...
     it('should emit cache-revalidate-error on revalidation failure', async () => {
        ...
        api.on('cache-revalidate-error', (data) => errorEvents.push(data));
    ...
    it('should invalidate all when predicate always returns true', async () => {
     ...
     it('should deduplicate correctly without cache enabled', async () => {
        ...
        api.on('dedupe-start', () => startEvents.push('start'));
        api.on('dedupe-join', () => joinEvents.push('join'));
    ...
    describe('cache + deduplication integration', () => {
        ...
        it('should handle cache hit preventing deduplication need', async () => {
            ...
            let dedupeStarts = 0;
            let cacheHits = 0;
            api.on('dedupe-start', () => { dedupeStarts++; });
            api.on('cache-hit', () => { cacheHits++; });
    ...
    describe('TTL/staleIn combinations', () => {
        ...
        it('should handle short TTL with no staleIn', async () => {
            ...
            const events: string[] = [];
            api.on('cache-miss', () => events.push('cache-miss'));
            api.on('cache-hit', () => events.push('cache-hit'));
        ...
        it('should handle long TTL with short staleIn', async () => {
            ...
            api.on('cache-miss', () => events.push('cache-miss'));
            api.on('cache-hit', () => events.push('cache-hit'));
            api.on('cache-stale', () => events.push('cache-stale'));
        ...
        it('should handle per-rule TTL overrides', async () => {
            ...
            api.on(/cache-/, (eventData) => {

                if ((eventData.data as any).path) {

                    events.push({ path: (eventData.data as any).path, event: eventData.event });
                }
            });
            ...
            expect(fastEvent?.event).to.equal('cache-miss');
            expect(slowEvent?.event).to.equal('cache-hit');
    ...
    describe('combined features', () => {
        ...
        it('should work with both dedupePolicy and cachePolicy enabled', async () => {
            ...
            api.on('cache-miss', () => events.push('cache-miss'));
            api.on('cache-set', () => events.push('cache-set'));
            api.on('cache-hit', () => events.push('cache-hit'));
        ...
        it('should handle GET with all features', async () => {
            ...
            const events: string[] = [];
            api.on(/dedupe|cache|retry/, (data) => {
        ...
        it('should handle mixed method configurations in single instance', async () => {
            ...
            const events: string[] = [];
            api.on(/dedupe|cache/, (data) => events.push(data.event));
        ...
        it('should handle concurrent requests with mixed cache states', async () => {
            ...
            const events: string[] = [];
            api.on(/cache-/, (data) => events.push(data.event));
            ...
            const staleCount = events.filter(e => e === 'cache-stale').length;
        ...
        it('should handle rule priority with overlapping patterns', async () => {
            ...
            expect(adminEvent?.event).to.equal('cache-miss');
            expect(apiEvent?.event).to.equal('cache-hit');
    ...
    describe('cache invariants', () => {
        ...
        it('should never have negative cacheSize', async () => {
            ...
            const key = await new Promise<string>((resolve) => {
                ...
                api.on('cache-set', (data) => resolve(data.key));
                api.get('/json3');
            });
        ...
        it('should handle cache hit while dedupe in flight', async () => {
            ...
            api.on('cache-hit', () => cacheHitEvents.push('hit'));
        ...
        it('should deduplicate cache misses correctly', async () => {
            ...
            api.on('dedupe-start', () => dedupeStartEvents.push('start'));
            api.on('dedupe-join', () => dedupeJoinEvents.push('join'));
            api.on('cache-miss', () => cacheMissEvents.push('miss'));
    ...
    describe('cache event interfaces', () => {
        ...
        it('should emit cache-miss with correct interface', async () => {
        ...
        it('should emit cache-hit with correct interface', async () => {
        ...
        it('should emit cache-set with correct interface', async () => {
        ...
        it('should emit cache-stale with correct interface when using SWR', async () => {
        ...
        it('should emit cache-revalidate with correct interface', async () => {
        ...
        it('should emit cache-expire with correct interface', async () => {
    ...
    describe('cache performance', () => {
        ...
        it('should handle errors during cache serialization', async () => {
            ...
            const [, err] = await attempt(() => api.get(path));
    ...
    describe('invalid callback inputs', () => {
        ...
        it('should handle skip callback that throws', async () => {
            ...
            api.on('error', ({ data }: FetchEngine.EventData) => {
