...
describe('@logosdx/fetch: rate limiting', async () => {
    ...
    const { testUrl } = await makeTestStubs(4300);
    ...
    describe('integration with cache and deduplication', () => {
        ...
        it('should check cache before rate limit', async () => {
            ...
            // First request: cache miss -> rate limit -> fetch
            await api.get('/json');

            expect(rateLimitEvents).to.deep.equal(['/json']);
            expect(cacheEvents).to.deep.equal(['miss:/json']);

            // Second request: cache hit -> returns immediately (no rate limit consumed)
            await api.get('/json');
