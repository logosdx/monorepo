...
describe('@logosdx/fetch: deduplication',  async () => {
    ...
    it('should handle in-flight requests when engine is destroyed mid-flight', async () => {
        ...
        api.on('cache-set', () => cacheSetEvents.push('set'));
    ...
    it('should handle empty/root path', async () => {
        ...
        api2.on('dedupe-start', (data) => startEvents2.push(data.path!));
    ...
    it('should NOT deduplicate requests with different request-specific params', async () => {
        ...
        api.on('dedupe-start', (data) => startEvents.push(data.key));
    ...
    it('should deduplicate joiners that arrive during retry attempts', async () => {
        ...
        api.on('dedupe-start', () => events.push('start'));
        api.on('dedupe-join', () => events.push('join'));
    ...
    describe('rule resolution and config paths', () => {
        ...
        it('should evaluate rules separately for different paths', async () => {
            ...
            expect(startEvents.length).to.equal(2);
            expect((r1.data as any).ok).to.be.true;
            expect((r2.data as any).ok).to.be.true;
            expect((r3.data as any).ok).to.be.true;
            expect((r4.data as any).ok).to.be.true;
        ...
        it('should handle rule inheritance and override patterns', async () => {
            ...
            // Verify results
            expect((i1.data as any).ok && (i2.data as any).ok && (i3.data as any).ok && (i4.data as any).ok).to.be.true;
            expect((o1.data as any).ok && (o2.data as any).ok && (o3.data as any).ok && (o4.data as any).ok).to.be.true;
    ...
    describe('path boundaries', () => {
        ...
        it('should handle empty path ""', async () => {
            ...
            api.on('dedupe-start', ({ path }: any) => {

                events.push(`start:${path}`);
            });

            api.on('dedupe-join', ({ path }: any) => {
    ...
    describe('event interfaces', () => {
        ...
        it('should emit dedupe-start with correct interface', async () => {
        ...
        it('should emit dedupe-join with correct interface', async () => {
        ...
        it('should emit dedupe-complete with correct interface', async () => {
        ...
        it('should emit dedupe-error with correct interface on failure', async () => {
