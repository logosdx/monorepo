...
describe('@logosdx/utils - Inflight Deduplication', () => {
    ...
    it('should not double wrap the function', async () => {
        ...
        const wrappedFn = withInflightDedup(fn);
        const [, error] = attemptSync(() => withInflightDedup(wrappedFn));
    ...
    describe('single flight per key', () => {
        ...
        it('should execute producer once for concurrent calls with same args', async () => {
            ...
            const producer = vi.fn(async (id: string) => {
    ...
    describe('resolution lifecycle', () => {
        ...
        it('should fire onResolve once per key', async () => {
            ...
            expect(resolveCall?.[0]).to.match(/42/); // key contains '42'
            expect(resolveCall?.[1]).to.equal('result-42'); // value
        });
    ...
    describe('rejection lifecycle', () => {
        ...
        it('should fire onReject once per key', async () => {
            ...
            expect(rejectCall?.[0]).to.match(/42/); // key
            expect(rejectCall?.[1]).to.be.instanceOf(Error); // error
        });
    ...
    describe('custom key function', () => {
        ...
        it('should use custom generateKey when provided', async () => {
            ...
            const producer = vi.fn(async (id: string, _opts: { timestamp: number }) => {
        ...
        it('should handle function arguments with custom generateKey', async () => {
            ...
            const producer = vi.fn(async (url: string, transform: (x: any) => any) => {
        ...
        it('should support complex custom keys', async () => {
            ...
            const producer = vi.fn(async (req: Request) => {
    ...
    describe('default serializer correctness', () => {
        ...
        it('should dedupe identical primitive args', async () => {
            ...
            const producer = vi.fn(async (a: number, b: string, c: boolean) => {
        ...
        it('should dedupe objects with same structure regardless of key order', async () => {
            ...
            const producer = vi.fn(async (obj: any) => {
        ...
        it('should dedupe arrays with same values', async () => {
            ...
            const producer = vi.fn(async (arr: number[]) => {
        ...
        it('should dedupe Dates with same timestamp', async () => {
            ...
            const producer = vi.fn(async (date: Date) => {
        ...
        it('should dedupe RegExp with same pattern and flags', async () => {
            ...
            const producer = vi.fn(async (regex: RegExp) => {
        ...
        it('should dedupe Maps with same entries regardless of insertion order', async () => {
            ...
            const producer = vi.fn(async (map: Map<string, number>) => {
        ...
        it('should dedupe Sets with same values regardless of insertion order', async () => {
            ...
            const producer = vi.fn(async (set: Set<number>) => {
        ...
        it('should handle circular references without throwing', async () => {
            ...
            // Should not throw - different circular objects are correctly identified as distinct
            ...
            expect(r3).to.equal(42);
            expect(r4).to.equal(42);

            calledExactly(producer, 2, 'different circular objects not falsely deduped');
        });
    });
    ...
    describe('edge cases', () => {
        ...
        it('should handle null and undefined', async () => {
            ...
            const producer = vi.fn(async (val: any) => {
        ...
        it('should distinguish -0 from 0', async () => {
            ...
            const producer = vi.fn(async (n: number) => {
        ...
        it('should handle nested complex structures', async () => {
            ...
            const producer = vi.fn(async (data: any) => {
    ...
    describe('conditional deduplication (shouldDedupe)', () => {
        ...
        it('should bypass deduplication when shouldDedupe returns false', async () => {
            ...
            const producer = vi.fn(async (id: string, _opts?: { bustCache?: boolean }) => {
        ...
        it('should not invoke serializer when shouldDedupe returns false', async () => {
            ...
            const generateKey = vi.fn((id: string, _opts?: { bustCache?: boolean }) => id);
        ...
        it('should pass all arguments to shouldDedupe', async () => {
            ...
            const producer = vi.fn(async (a: string, b: number, c: boolean) => {

                await wait(10);
                return `${a}-${b}-${c}`;
            });

            const shouldDedupe = vi.fn((_a: string, _b: number, _c: boolean) => true);
