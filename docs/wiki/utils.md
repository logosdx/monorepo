...
- `PathValue<T, P>` in [`packages/utils/src/types.ts`](../../packages/utils/src/types.ts) wraps its whole body in a bare `T extends any` conditional — a naked type-parameter check with no meaningful branch difference, whose only effect is forcing TypeScript to distribute over a union in `T` (mirrors the same technique in `PathNames`) before the `keyof T`/`Map`/`Set`/array branches run. This makes a dotted path into a union type (e.g. `RetryConfig | false`) resolve against the matching member instead of collapsing to `never`.
