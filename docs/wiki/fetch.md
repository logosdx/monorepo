...
- [`skills/logosdx/references/fetch.md`](../../skills/logosdx/references/fetch.md) — skill reference (1238 LOC) covering plugins, policies, hooks, events
...
- [`docs/packages/fetch/policies.md`](../packages/fetch/policies.md) — resilience policy docs (1105 LOC); includes a per-call escape-hatch code example (`retry: false`, `skipCache: true`) around line 88
...
- Tests in [`tests/src/fetch/`](../../tests/src/fetch) cover engine, cookies, policies, serializers, state, adapters; [`tests/src/fetch/engine/plugin-resolution.test.ts`](../../tests/src/fetch/engine/plugin-resolution.test.ts) (5 tests) specifically covers config-key/`plugins` array composition and the construction-time conflict throws. [`tests/src/fetch/executor/per-call-overrides.test.ts`](../../tests/src/fetch/executor/per-call-overrides.test.ts) (3 tests) covers per-call `retry: false` overriding an engine configured with retries, per-call `retry` config overriding an engine configured with `retry: false`, and per-call `skipCache` bypassing both cache lookup and store.
