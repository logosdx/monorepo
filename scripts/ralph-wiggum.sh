...
#   2. ./scripts/ralph-wiggum.sh --login    # Authenticate Claude inside container
#
...
IMAGE_NAME="${RALPH_IMAGE:-ralph-wiggum:$REPO_NAME}"
...
if [[ -z "$PROMPT_OR_FILE" ]]; then
    ...
    echo "  --login    Authenticate Claude inside the container"
